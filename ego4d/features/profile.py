...
def profile_extraction(config: FeatureExtractConfig):
    ...
    for batch_size, num_workers, prefetch_factor in zip(batch_sizes, num_workers, prefetch_factor):
        config.inference_config.batch_size = batch_size
        config.inference_config.num_workers = num_workers
        config.inference_config.prefetch_factor = prefetch_factor
        ...
        if batch_size == 0:
            print(
                f"{prefetch_factor},{batch_size},{num_workers},{total_time},{mean_sum},{forward_pass.mean()},{to_load.mean()},{transfer_time.mean()}"
            )
        else:
            print(
                f"{prefetch_factor},{batch_size},{num_workers},{total_time},{mean_sum},{forward_pass.mean()/batch_size},{to_load.mean()/batch_size},{transfer_time.mean()/batch_size}"
            )
